...
const BasePath = "/azure-ai/:resource_name/openai/deployments/:deployment_name"
const ProviderName = "azure-openai"
const azureAIRoute = new Hono()
...
azureAIRoute.post("/*", async (c: Context) => {
	return azureAIProvider.handleRequest(c)
})

export const azureAIProvider: AIProvider = {
	name: ProviderName,
	basePath: BasePath,
	route: azureAIRoute,
	...
	handleRequest: async (c: Context) => {
		...
		const functionName = c.req.path.slice(`/azure-ai/${resourceName}/openai/deployments/${deploymentName}/`.length)
		const azureEndpoint = `https://${resourceName}.cognitiveservices.azure.com/openai/deployments/${deploymentName}/${functionName}`

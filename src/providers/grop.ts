...
groqRoute.post("/*", async (c: Context) => {
	return groqProvider.handleRequest(c)
})

groqRoute.get("/*", async (c: Context) => {
	return c.text("Groq endpoint on Malacca.", 200, { "Content-Type": "text/plain" })
})
...
export const groqProvider: AIProvider = {
	...
	handleRequest: async (c: Context) => {
		const functionName = c.req.path.slice("/groq/".length)
		const openaiEndpoint = `https://api.groq.com/openai/${functionName}`
